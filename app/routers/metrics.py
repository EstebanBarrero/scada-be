...
def get_top_tags(
    ...
    """Top N tags ranked by total alarm event count. Rate limit: 30/minute per IP."""
    return MetricsService(db).get_top_tags(limit=limit)


@router.get("/by-criticality", response_model=CriticalityResponse)
@limiter.limit("30/minute")
def get_by_criticality(request: Request, db: Session = Depends(get_db)):
    """Alarm count and percentage breakdown by criticality level. Rate limit: 30/minute per IP."""
